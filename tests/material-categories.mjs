...
try {
 ...
 await page.goto(base+'/app');await page.waitForSelector('#browse-all');await page.locator('#browse-all').click();await page.waitForSelector('[data-kind-filter]');
 check(await page.locator('[data-kind-filter]').count()===6,'Library has All plus explanations, diseases, care, cost and cases');
 ...
 await page.locator('[data-cat="소아치과"]').click();
 ...
 check(await page.locator('#editor [data-kind]').count()===5,'Editor offers care instructions as a selectable type');
 check((await page.locator('#editor').innerText()).includes('치료 전후 관리'),'Legacy precaution editor explains compatibility');
 ...
 await page.reload();await page.waitForSelector('#browse-all');await page.locator('#browse-all').click();await page.waitForSelector('[data-kind-filter]');await page.locator('[data-kind-filter="disease"]').click();
