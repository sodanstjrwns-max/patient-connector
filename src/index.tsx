...
app.use("*", async (c, next) => {
  ...
  c.header(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; font-src 'self' https://cdn.jsdelivr.net data:; img-src 'self' https: data: blob:; media-src 'self' https: blob:; connect-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'none'; form-action 'self'",
  );
  if (
  ...
  bodyLimit({
    ...
    onError: (c) =>
      c.json(
        {
          error:
            "요청이 너무 큽니다. 파일은 10MB, 상담은 40장 이내로 나누어 주세요.",
        },
        413,
      ),
  }),
...
app.use("/api/*", async (c, next) => {
  if (
    ...
    if (!b || typeof b !== "object" || Array.isArray(b))
      return c.json({ error: "올바른 JSON 객체가 필요합니다." }, 400);
  }
  await next();
});
app.onError((err, c) => {
  ...
  if (err instanceof SyntaxError)
    return c.json({ error: "입력 형식을 확인해 주세요." }, 400);
  console.error(
    "Request failed",
    c.req.method,
    c.req.path.replace(/\/share\/[^/]+/, "/share/[token]"),
    err.name,
  );
  return c.json(
...
app.get("/ph", (c) => {
  ...
  const title = esc((c.req.query("t") || "치료 설명자료").slice(0, 60)),
    sub = esc((c.req.query("s") || "").slice(0, 100));
...
app.post("/api/auth/login", async (c) => {
  const b = await c.req.json(),
    email = text(b.email, 254, true).toLowerCase(),
    password = passwordValue(b.password);
  await rateLimit(
    c,
  ...
  if (
    (email === "admin@patientconnect.kr" && password === "admin1234") ||
    (email === "demo@clinic.com" && password === "demo1234")
  )
    return c.json(
      {
        error:
          "기존 공개 데모 비밀번호는 보안상 사용이 중지되었습니다. 새 병원 계정을 만들거나 운영자에게 비밀번호 재설정을 요청해 주세요.",
      },
      401,
    );
...
app.post("/api/auth/signup", async (c) => {
  await rateLimit(
    c,
    "signup:" + (c.req.header("CF-Connecting-IP") || "local"),
    8,
    3600,
  );
  const b = await c.req.json(),
    ...
    clinic = text(b.clinic_name, 150, true);
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
  ...
  if (
    await c.env.DB.prepare("SELECT id FROM users WHERE lower(email)=?")
      .bind(email)
      .first()
  )
  ...
  const res = await c.env.DB.batch([
    c.env.DB.prepare(
      "INSERT INTO clinics(name,phone,specialty) VALUES(?,?,?)",
    ).bind(clinic, text(b.phone, 30), b.specialty),
    c.env.DB.prepare(
      "INSERT INTO users(clinic_id,email,password_hash,name,role) VALUES(last_insert_rowid(),?,?,?,'owner')",
    ).bind(email, hash, name),
  ]);
  await newSession(c, Number(res[1].meta.last_row_id));
...
app.post("/api/auth/password", async (c) => {
  const u = await requireUser(c),
    b = await c.req.json();
  await rateLimit(c, `password:${u.id}`, 6);
  const row = await c.env.DB.prepare(
    "SELECT password_hash FROM users WHERE id=?",
  )
    .bind(u.id)
    .first<any>();
  if (!(await verifyPassword(b.current_password, row.password_hash)))
    fail("현재 비밀번호가 올바르지 않습니다.");
  const pw = passwordValue(b.password);
  if (pw.length < 10) fail("새 비밀번호는 10자 이상이어야 합니다.");
  await c.env.DB.batch([
    c.env.DB.prepare("UPDATE users SET password_hash=? WHERE id=?").bind(
      await hashPassword(pw),
      u.id,
    ),
    c.env.DB.prepare("DELETE FROM auth_sessions WHERE user_id=?").bind(u.id),
  ]);
  await newSession(c, u.id);
  return c.json({ ok: true });
});
app.get("/api/clinic", async (c) => {
  const u = await requireUser(c);
  return c.json({
    clinic: await c.env.DB.prepare("SELECT * FROM clinics WHERE id=?")
      .bind(requireClinic(u))
      .first(),
  });
});
app.put("/api/clinic", async (c) => {
  const u = await requireUser(c),
    b = await c.req.json();
  if (u.role !== "owner") fail("대표 계정만 변경할 수 있습니다.", 403);
  if (!specialties.includes(b.specialty)) fail("진료과를 확인해 주세요.");
  await c.env.DB.prepare(
    "UPDATE clinics SET name=?,phone=?,address=?,emergency_info=?,specialty=? WHERE id=?",
  )
    .bind(
      text(b.name, 150, true),
      text(b.phone, 30),
      text(b.address, 300),
      text(b.emergency_info, 1000),
      b.specialty,
      requireClinic(u),
    )
    .run();
...
app.get("/api/treatments", async (c) => {
  const sp = c.req.query("specialty");
  const { results } = await c.env.DB.prepare(
    "SELECT * FROM treatments WHERE (? IS NULL OR specialty=?) ORDER BY sort_order,id",
  )
    .bind(sp || null, sp || null)
    .all();
...
app.get("/api/assets", async (c) => {
  const u = await getUser(c),
    ...
    manage = c.req.query("manage") === "1";
  let sql =
    "SELECT a.*,t.name as treatment_name,t.specialty,EXISTS(SELECT 1 FROM favorites f WHERE f.asset_id=a.id AND f.user_id=?) as favorite FROM assets a LEFT JOIN treatments t ON t.id=a.treatment_id WHERE ";
  const binds: any[] = [u?.id || 0];
  if (manage && u?.role === "admin") sql += "a.is_public=1";
  else if (manage && u?.clinic_id) {
    sql += "a.clinic_id=?";
    binds.push(u.clinic_id);
  } else {
    sql += "a.is_hidden=0 AND (a.is_public=1 OR a.clinic_id=?)";
    binds.push(u?.clinic_id || 0);
  }
  for (const [param, col] of [
    ...
    if (v && v !== "all") {
      sql +=
        param === "specialty"
          ? ` AND (${col}=? OR a.treatment_id IS NULL)`
          : ` AND ${col}=?`;
      binds.push(v);
    }
  }
  ...
  if (c.req.query("favorite") === "1") {
    sql +=
      " AND EXISTS(SELECT 1 FROM favorites f WHERE f.asset_id=a.id AND f.user_id=?)";
    binds.push(u?.id || 0);
  }
  if (q) {
    sql += " AND (a.title LIKE ? OR a.description LIKE ? OR a.tags LIKE ?)";
    binds.push(...Array(3).fill(`%${q}%`));
  }
  const order =
    c.req.query("sort") === "popular"
      ? "a.use_count DESC"
      : c.req.query("sort") === "new"
        ? "a.id DESC"
        : "a.sort_order,a.id";
  const { results } = await c.env.DB.prepare(
...
app.post("/api/assets/:id/use", async (c) => {
  ...
  await rateLimit(
    c,
    "use:" + (c.req.header("CF-Connecting-IP") || "local"),
    100,
    60,
  );
  await c.env.DB.prepare("UPDATE assets SET use_count=use_count+1 WHERE id=?")
    .bind(a.id)
    .run();
  return c.json({ ok: true });
});
...
app.put("/api/assets/:id", async (c) => {
  const u = await requireUser(c),
    raw = await assetFor(c.env, c.req.param("id"), u);
...
app.delete("/api/assets/:id", async (c) => {
  const u = await requireUser(c),
    a = await assetFor(c.env, c.req.param("id"), u);
  if (!canEdit(a, u)) fail("삭제 권한이 없습니다.", 403);
  await c.env.DB.batch([
    c.env.DB.prepare("DELETE FROM favorites WHERE asset_id=?").bind(a.id),
    c.env.DB.prepare("DELETE FROM assets WHERE id=?").bind(a.id),
  ]);
  return c.json({ ok: true });
});
app.post("/api/assets/:id/duplicate", async (c) => {
  const u = await requireUser(c),
    ...
    a = await assetFor(c.env, c.req.param("id"), u);
  const r = await c.env.DB.prepare(
    "INSERT INTO assets(clinic_id,source_asset_id,treatment_id,category,type,title,description,media_urls,payload,reviewer_name,tags,is_public) VALUES(?,?,?,?,?,?,?,?,?,?,?,0)",
  )
    .bind(
      clinic,
      a.id,
...
app.post("/api/assets/:id/favorite", async (c) => {
  ...
  await c.env.DB.prepare(
    b.favorite
      ? "INSERT OR IGNORE INTO favorites(user_id,asset_id) VALUES(?,?)"
...
app.post("/api/upload", async (c) => {
  const u = await requireUser(c);
  await rateLimit(c, `upload:${u.id}`, 60, 3600);
  const form = await c.req.formData(),
    file = form.get("file");
  if (!(file instanceof File)) fail("파일을 선택해 주세요.");
  if (file.size > 10 * 1024 * 1024)
    fail("파일은 10MB 이하로 업로드해 주세요.", 413);
  ...
  if (!ext) fail("JPG, PNG, WebP, MP4, WebM 파일만 지원합니다.");
  const bytes = new Uint8Array(await file.arrayBuffer()),
    sig = String.fromCharCode(...bytes.slice(0, 12));
  const valid =
    file.type === "image/png"
      ? bytes[0] === 137 && sig.slice(1, 4) === "PNG"
      : file.type === "image/jpeg"
        ? bytes[0] === 255 && bytes[1] === 216
        : file.type === "image/webp"
          ? sig.startsWith("RIFF") && sig.slice(8) === "WEBP"
          : file.type === "video/mp4"
            ? sig.slice(4, 8) === "ftyp"
            : bytes[0] === 26 &&
              bytes[1] === 69 &&
  ...
  await c.env.DB.prepare(
    "INSERT INTO uploads(key,clinic_id,user_id,content_type,size) VALUES(?,?,?,?,?)",
  )
    .bind(key, u.clinic_id, u.id, file.type, file.size)
    .run();
  return c.json(
    { url: `/files/${key}`, name: file.name, type: file.type },
    201,
  );
});
app.get("/files/*", async (c) => {
  const key = c.req.path.slice(7),
    url = "/files/" + key;
  ...
  let allowed =
    u?.role === "admin" ||
    !!(u?.clinic_id && key.startsWith(`clinic${u.clinic_id}/`));
  if (!allowed) {
    const publicRef = await c.env.DB.prepare(
  ...
  if (!allowed && u?.clinic_id)
    allowed = !!(await c.env.DB.prepare(
      "SELECT id FROM assets WHERE clinic_id=? AND (instr(media_urls,?)>0 OR instr(payload,?)>0) LIMIT 1",
    )
      .bind(u.clinic_id, JSON.stringify(url), JSON.stringify(url))
      .first());
  if (!allowed && u?.clinic_id) {
    const sessions = await c.env.DB.prepare(
      "SELECT slides FROM consult_sessions WHERE clinic_id=? AND instr(slides,?)>0 LIMIT 50",
    )
      .bind(u.clinic_id, JSON.stringify(url))
      .all<any>();
    allowed = sessions.results.some((s) =>
      sharedFiles(JSON.parse(s.slides)).includes(url),
    );
  }
  const token = c.req.query("share");
  if (!allowed && token) {
    const s = await activeShare(c.env, token);
    if (s) allowed = sharedFiles(await shareSlides(c.env, s)).includes(url);
  }
  ...
  if (rangeHeader) {
    ...
    const start = m?.[1]
      ? Number(m[1])
      : m?.[2]
        ? Math.max(0, meta.size - Number(m[2]))
        : NaN;
    const end =
      m?.[1] && m[2] ? Math.min(Number(m[2]), meta.size - 1) : meta.size - 1;
    if (
  ...
  if (
    ...
    c.header("Content-Length", String(obj.range.length));
    return c.body(obj.body, 206);
  }
...
app.get("/api/cases", async (c) => {
  const u = await getUser(c),
    t = c.req.query("treatment"),
    sp = u?.clinic_specialty;
  const { results } = await c.env.DB.prepare(
    `SELECT cs.*,t.name as treatment_name FROM cases cs LEFT JOIN treatments t ON t.id=cs.treatment_id WHERE (cs.clinic_id IS NULL OR cs.clinic_id=?) AND (? IS NULL OR cs.treatment_id=?) AND (? IS NULL OR t.specialty=?) ORDER BY cs.id DESC LIMIT 300`,
  )
    .bind(
      u?.clinic_id || 0,
      t && t !== "all" ? t : null,
...
app.post("/api/cases", async (c) => {
  const u = await requireUser(c),
    ...
    b = await c.req.json();
  if (b.consent !== true)
    fail("사진의 원내 상담 사용에 대한 환자 동의를 확인해 주세요.");
  const before = mediaURL(b.before_url),
    after = mediaURL(b.after_url);
  if (!before || !after) fail("비포·애프터 사진 두 장이 필요합니다.");
  await checkMediaOwnership(c.env, [before, after], u);
  const r = await c.env.DB.prepare(
    "INSERT INTO cases(clinic_id,treatment_id,title,before_url,after_url,duration,material,doctor,consent) VALUES(?,?,?,?,?,?,?,?,1)",
  )
    .bind(
      clinic,
      b.treatment_id ? idOf(b.treatment_id) : null,
      text(b.title, 180, true),
      before,
      after,
      text(b.duration, 100),
      text(b.material, 150),
      text(b.doctor, 100) || u.name,
    )
    .run();
  return c.json({ ok: true, id: r.meta.last_row_id });
});
app.delete("/api/cases/:id", async (c) => {
  const u = await requireUser(c);
  const r = await c.env.DB.prepare(
    "DELETE FROM cases WHERE id=? AND clinic_id=?",
  )
    .bind(idOf(c.req.param("id")), requireClinic(u))
    .run();
  if (!r.meta.changes) fail("케이스를 찾을 수 없습니다.", 404);
  return c.json({ ok: true });
});
...
app.get("/api/sessions/:id", async (c) => {
  ...
  return c.json({ session: { ...s, slides: JSON.parse(s.slides) } });
});
app.delete("/api/sessions/:id", async (c) => {
  const u = await requireUser(c),
    id = idOf(c.req.param("id"));
  const s = await c.env.DB.prepare(
    "SELECT id FROM consult_sessions WHERE id=? AND clinic_id=?",
  )
    .bind(id, requireClinic(u))
    .first();
  if (!s) fail("상담을 찾을 수 없습니다.", 404);
  await c.env.DB.batch([
...
app.delete("/api/sessions/:id/share", async (c) => {
  const u = await requireUser(c);
  const r = await c.env.DB.prepare(
    "UPDATE consult_sessions SET share_revoked_at=datetime('now') WHERE id=? AND clinic_id=?",
  )
    .bind(idOf(c.req.param("id")), requireClinic(u))
    .run();
  if (!r.meta.changes) fail("상담을 찾을 수 없습니다.", 404);
  return c.json({ ok: true });
});
...
app.post("/api/share/:token/view", async (c) => {
  const token = c.req.param("token"),
    s = await activeShare(c.env, token);
  if (!s) fail("공유가 종료되었습니다.", 404);
  const u = await getUser(c);
  if (u?.clinic_id === s.clinic_id) return c.json({ ok: true });
  const b = await c.req.json(),
    visitor = text(b.visitor, 100, true);
  await rateLimit(c, `view:${s.id}:${visitor}`, 1, 1800);
  await c.env.DB.prepare("INSERT INTO share_views(session_id) VALUES(?)")
    .bind(s.id)
    .run();
  return c.json({ ok: true });
});
app.get("/api/dashboard", async (c) => {
  const u = await requireUser(c),
    clinic = requireClinic(u);
  const rows = await c.env.DB.batch<{ count: number }>([
    c.env.DB.prepare(
      "SELECT COUNT(*) as count FROM assets WHERE clinic_id=?",
    ).bind(clinic),
    c.env.DB.prepare(
      "SELECT COUNT(*) as count FROM consult_sessions WHERE clinic_id=?",
    ).bind(clinic),
    c.env.DB.prepare(
      "SELECT COUNT(DISTINCT s.id) as count FROM consult_sessions s JOIN share_views v ON v.session_id=s.id WHERE s.clinic_id=?",
    ).bind(clinic),
    c.env.DB.prepare(
  ...
  return c.json({
    assets: rows[0].results[0].count,
    sessions: rows[1].results[0].count,
    viewed: rows[2].results[0].count,
    favorites: rows[3].results[0].count,
  });
});
app.get("/api/admin/stats", async (c) => {
  const u = await requireUser(c);
  if (u.role !== "admin") fail("운영자 권한이 필요합니다.", 403);
  const assets = await c.env.DB.prepare(
    "SELECT a.*,t.name as treatment_name FROM assets a LEFT JOIN treatments t ON t.id=a.treatment_id WHERE a.is_public=1 ORDER BY a.use_count DESC",
  ).all();
  const clinics = await c.env.DB.prepare(
      "SELECT COUNT(*) as cnt FROM clinics",
    ).first<any>(),
    sessions = await c.env.DB.prepare(
      "SELECT COUNT(*) as cnt FROM consult_sessions",
    ).first<any>();
  return c.json({
    assets: assets.results.map(parseAsset),
    clinic_count: clinics?.cnt || 0,
...
app.get("/journey", (c) =>
  c.html(pageNotFound("치료 여정 타임라인은 준비 중입니다")),
);
app.get("/consent", (c) =>
  c.html(pageNotFound("진료 동의 서명은 준비 중입니다")),
);
app.get("/market", (c) =>
  c.html(pageNotFound("자료 공유 마켓은 준비 중입니다")),
);
app.notFound((c) =>
  c.html(pageNotFound("요청하신 페이지를 찾을 수 없습니다"), 404),
);
export default app;
