...
- **디자인 컨셉 (v2 AURORA DARK)**: 전면 다크 글래스모피즘 — 오로라 그라데이션 배경 + 글래스 카드 + 그라데이션 텍스트/보더. 화이트라벨: 로그인하면 병원 이름이 헤더에 그라데이션으로 크게 표시되어 "그 병원 전용 프로그램"처럼 보임.
...
## 완료된 기능
0. **다중 진료과 지원** — 치과 / 피부·미용 / 성형외과 / 정형·재활 / 안과 / 한방. 홈에서 진료과 선택 칩(localStorage 기억), 가입 시 병원 진료과 선택
