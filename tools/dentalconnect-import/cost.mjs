...
const BASIS = '2026.09'
const PHOTO_DIR = path.resolve(path.dirname(new URL(import.meta.url).pathname), '../costsrc/final')
const MIME = { png: 'image/png', jpg: 'image/jpeg', webp: 'image/webp' }
const IMG = n => `data:${MIME[n.split('.').pop()]};base64,${fs.readFileSync(path.join(PHOTO_DIR, n)).toString('base64')}`
const pic = (n, cap = '', cls = '') => `<figure class="pic ${cls}"><img src="${IMG(n)}" alt="">${cap ? `<figcaption>${cap}</figcaption>` : ''}</figure>`
...
body{margin:0;width:1600px;height:1000px;font-family:Pretendard,'Apple SD Gothic Neo','Noto Sans KR',sans-serif;background:#f6f1ea;color:#1f1a17;-webkit-font-smoothing:antialiased}
.page{position:relative;width:1600px;height:1000px;overflow:hidden;display:flex;flex-direction:column;background:#f6f1ea}
...
.row .ri{width:220px;height:146px;object-fit:cover;border-radius:12px;flex:none;background:#faf7f2;border:1px solid #efe7dc}
.row .ri.contain{object-fit:contain;padding:6px;background:#fff}
.rows.sm .ri{width:146px;height:96px}
.rows.sm .row{padding:12px 22px}
.rows.sm .row .n{font-size:23px}.rows.sm .row .d{font-size:15.5px}.rows.sm .row .p{font-size:28px}
.pic{margin:0;background:#fff;border:1px solid #e6dccf;border-radius:18px;overflow:hidden;box-shadow:0 2px 10px rgba(59,42,30,.05);display:flex;flex-direction:column}
.pic img{width:100%;flex:1;min-height:0;object-fit:cover;display:block}
.pic.contain img{object-fit:contain;padding:14px;background:#fff}
.pic figcaption{padding:9px 12px;font-size:14px;color:#7a6f65;text-align:center;border-top:1px solid #f1ebe2}
.pic.h200{height:200px}.pic.h260{height:260px}.pic.h330{height:330px}.pic.h420{height:420px}.pic.fill{flex:1;min-height:0}
...
const rows = (items, opt = {}) => `<div class="rows${opt.sm ? ' sm' : ''}">${items.map(r => `<div class="row${r.hi ? ' hi' : ''}">${r.img ? `<img class="ri${r.fit === 'contain' ? ' contain' : ''}" src="${IMG(r.img)}" alt="">` : ''}<div>${r.tag ? `<span class="tag${r.tagGold ? ' gold' : ''}">${esc(r.tag)}</span>` : ''}<span class="n">${esc(r.name)}</span>${r.desc ? `<div class="d">${r.desc}</div>` : ''}</div><div class="p">${r.was ? `<span class="strike">${won(r.was)}</span>` : ''}${typeof r.price === 'number' ? won(r.price) : esc(r.price)}${r.unit ? `<small>${esc(r.unit)}</small>` : ''}</div></div>`).join('')}</div>`
...
const PAGES = [
  ...
  { slug: 'implant-1', title: '임플란트 비용 안내', sub: '비디치과는 오래가는 임플란트를 목표로 합니다', body: `<div class="col">${sec('임플란트 1개 기준')}${rows([
    { img: 'straumann_blx.png', fit: 'contain', tag: '세계 1위 · 스위스', tagGold: true, name: '스트라우만 BLX', desc: '록솔리드 재질(티타늄보다 1.8배 단단) · SLA 액티브 표면처리로 빠른 회복', price: 160, hi: true },
    { img: 'osstem_soi.jpg', fit: 'contain', tag: '국내 1위 · 최상위 라인', name: '오스템 SOI', desc: '뛰어난 유지력과 초기 안정성 · 인체 친화적 표면 처리로 빠른 골유착', price: 100 },
    { img: 'osstem_ca.jpg', fit: 'contain', tag: '국내 1위 · 기본형', name: '오스템 CA', desc: '오랜 기간 증명된 안정성 · 칼슘 이온 처리로 골유착 속도 향상', price: 80 },
  ...
  ])}${pic('care4.jpg', '레진홀 충전 · 컨택 타이트닝 · 보철물 재제작(어벗 포함) · 보철물 재제작(어벗 제외)', 'h200')}<div class="grid3">${card('본원 환자 특전', bul(['수술 후 1년까지는 모든 유지관리 항목이 100% 무상 지원됩니다.']))}${card('정기 검진의 중요성', bul(['보증 혜택은 본원이 안내하는 정기 검진 주기를 준수하실 때 지속 적용됩니다.']))}${card('타 병원 임플란트', bul(['제품 종류(오스템·스트라우만 등)와 부품 호환 여부 확인이 먼저 필요하므로 상담 후 진행합니다.']))}</div></div>` },
  ...
  { slug: 'posterior-1', title: '어금니 보철 비용', sub: '크라운 · 오버레이 · 인레이 (치아 1개 기준)', body: `<div class="col">${rows([
    { img: 'zir_crown.jpg', tag: '크라운', name: '지르코니아 크라운', desc: '독일 Amann Girrbach 오스트리아 직영 공장 · 100% 토소(Tosoh) 분말 블록', price: 55 },
    { img: 'gold_crown.jpg', tag: '크라운', name: '골드 크라운', desc: '최고등급 S0 type (금 함량 56%) 합금 사용', price: 95 },
    ...
    { img: 'gold_inlay.jpg', tag: '인레이', name: '골드 인레이', desc: '최고등급 I type (금 함량 83%) 합금 사용', price: 48 },
  ], { sm: true })}</div><div class="col" style="flex:.55">${sec('보철 보증 기간')}${warr(WARR_GEN)}${card('보증 안내', bul(['일반 보철(크라운·인레이) 공통 보증 기준입니다.', '정기 검진 주기를 지키실 때 보증이 유지됩니다.']))}</div>` },
  { slug: 'posterior-2', title: '어금니 보철 재료 안내', sub: '서울비디치과가 재료를 고르는 기준', body: `<div class="col"><div class="grid3">${card('지르코니아', pic('zolid_zi.png', 'Amann Girrbach 지르코니아 블록 (100% 토소 분말)', 'contain h260') + bul(['아주 단단해 보철물로 적합하며 치아색과 유사해 심미적입니다.', '지르코니아 블록을 깎아서 만들며, 필요한 치아 삭제량은 금보다 큽니다.', '국내 유통 블록 상당수는 중국산·국내산 분말입니다. 본원은 독일 Amann Girrbach 오스트리아 직영 공장의 100% 토소 분말 블록을 사용합니다.']))}${card('세라믹 (e.max)', pic('ceramic_inlay.jpg', 'Ivoclar e.max 세라믹 인레이', 'h260') + bul(['아주 단단하고 치아색과 유사해 심미적으로 뛰어납니다.', '필요한 치아 삭제량은 금보다 큽니다.', '본원은 매우 심미적이고 튼튼한 Ivoclar vivadent e.max로 인레이·오버레이를 제작합니다.']))}${card('골드', pic('gold_crown.jpg', '최고등급 골드 합금 크라운', 'h260') + bul(['잘 늘어나는 성질로 치아 삭제량이 적고 미세 부위를 잘 맞출 수 있으며 교합력에 적응하는 좋은 재료입니다.', '일반 치과는 크라운 A type(금 46%)·A4G(33.6%), 인레이 P type(77.2%)을 씁니다. 본원은 최고등급 크라운 S0 type(56%)·인레이 I type(83%)을 사용합니다.', '심미적이지 못한 단점이 있습니다.']))}</div></div>` },
  // ── 앞니 보철
  { slug: 'anterior', title: '앞니 보철 비용', sub: '전치부 크라운 (치아 1개 기준)', body: `<div class="col">${rows([
    { img: 'pfz_crowns.jpg', tag: '프리미엄', tagGold: true, name: '지르코니아 포세린 크라운 (PFZ)', desc: '지르코니아 위에 포세린을 쌓아 자연치의 투명감을 재현', price: 80, hi: true },
    { img: 'zir_crowns_ant.jpg', name: '지르코니아 크라운', desc: '단단하고 심미적인 풀 지르코니아 크라운', price: 60 },
  ...
  ])}${sec('보증 기간')}${warr(WARR_GEN)}</div><div class="col" style="flex:.7">${pic('glow_model.jpg', '', 'h330')}${card('글로우 리페어 진행 방식', bul(['원장단 협진을 통한 고난도 문제 해결 및 복원 시스템', '원장 협진팀의 정밀 진단 후 맞춤 견적을 안내합니다.']))}${card('디자인부터 사후 관리까지', bul(['하나의 작품으로 책임지는 작품 보증 시스템으로 관리합니다.', '다섯 가지 약속은 다음 장을 참고해 주세요.']))}</div>` },
  ...
  { slug: 'ortho-1', title: '치아교정 비용', sub: '고정식 교정 · 부분 교정 · 악궁 확장 · 악정형', body: `<div class="col">${sec('고정식 교정 장치 치료')}${rows([
    { img: 'clippy_c.jpg', name: '클리피씨 (Clippy-C)', desc: 'Tomy(일본) 세라믹 자가결찰 브라켓', price: 500 }, { img: 'clarity_ultra.png', fit: 'contain', name: '클라리티 울트라 (Clarity Ultra)', desc: '3M(미국) 투명 세라믹 자가결찰 브라켓', price: 550 },
  ])}${sec('상하악 전치 부분 교정')}${rows([{ name: '양악', price: 250 }, { name: '편악', price: 200 }])}</div><div class="col">${sec('악궁 확장 · 악정형 · 기타')}${rows([
    { img: 'ortho_expander.jpg', fit: 'contain', name: '악궁 확장 장치', desc: '아동 500,000원 · 성인 700,000원 (성인은 미니 임플란트 4개 비용 추가)', price: '50~70만원' },
  ...
  ])}</div><div class="col">${pic('invisalign.jpg', '인비절라인 투명 교정장치', 'contain h260')}${rows([
  ...
  { slug: 'tmj', title: '구강내과 비용', sub: '턱 보톡스 · 이갈이 장치', body: `<div class="col">${sec('턱 보톡스')}${rows([
    { img: 'wondertox.jpg', tag: '국내', name: '원더톡스', desc: '국내 생산 보툴리눔 톡신', price: 15 }, { img: 'xeomin.jpg', tag: '독일', tagGold: true, name: '제오민 (XEOMIN)', desc: '불순 단백질을 제거한 독일 프리미엄 보툴리눔 톡신', price: 35, hi: true },
  ])}</div><div class="col">${sec('이갈이 · 턱관절')}${rows([{ img: 'splint.jpg', name: '이갈이 장치 (스플린트)', desc: '맞춤 제작 · 월 진료비 10,000원 별도', price: 100 }])}${card('안내', bul(['이갈이 장치는 제작 후 정기 조정이 필요하며 월 진료비는 10,000원입니다.', '턱 보톡스는 개인별 근육량에 따라 용량이 조정될 수 있습니다.']))}</div>` },
  { slug: 'hygiene', title: '스테인 제거 · 지혈제', sub: '착색 제거와 발치·수술 시 사용하는 지혈 재료', body: `<div class="col">${sec('스테인 착색 제거')}${rows([{ img: 'ems_airflow.webp', name: '에어플로우 (EMS AIRFLOW)', desc: '고운 파우더 분사로 커피·담배 착색 제거 · 1악당', price: 5 }])}<div class="note" style="margin-top:4px">EMS AIRFLOW Prophylaxis Master 장비를 사용합니다.</div></div><div class="col">${sec('지혈제')}${rows([
    { img: 'cutanplast.jpg', name: '큐탄플라스트 (젤라틴 지혈 스펀지)', desc: '무균 젤라틴 스펀지 · 혈전 형성을 도와 자연 응고 유도 · 표준 재료', price: 1 },
    { img: 'bontree.jpg', tag: '프리미엄', tagGold: true, name: '본트리 큐브 (콜라겐 지혈 스펀지)', desc: 'Type 1 콜라겐 + 식물성 키토산 · 우수한 지혈 · 99.9% 항균 · 낮은 알러지 위험', price: 7, hi: true },
  ])}</div>` },
  { slug: 'event', eyebrow: '이벤트 안내', title: '이달의 이벤트', sub: '기간과 조건은 데스크에서 확인해 주세요', body: `<div class="col">${sec('턱 보톡스')}${rows([{ img: 'wondertox.jpg', tag: 'EVENT', tagGold: true, name: '원더톡스 (국내)', was: 15, price: 4.9, hi: true }])}</div><div class="col">${sec('원데이 치아미백')}${rows([{ img: 'whitening.jpg', tag: 'EVENT', tagGold: true, name: '1회 · 30분', was: 30, price: 4.9, hi: true }, { tag: 'EVENT', tagGold: true, name: '2회 · 60분', was: 60, price: 8, hi: true }])}</div>`, footNote: '이벤트 가격은 해당 기간에만 적용됩니다.' },
  // ── 주의사항(notice)
  ...
  { slug: 'partners', eyebrow: '제휴 혜택 안내', title: '제휴업체 혜택 안내', sub: '해당 기관 임직원·가족은 접수 시 미리 말씀해 주세요', body: `<div class="col">${pic('partners.jpg', '', 'contain h260')}<div class="chips">${['지방부수리', '선문대학교', '갤러리아 (the galleria)', '현대 (HYUNDAI)', 'we라이브병원', 'NIFCO', 'CITY GYM', 'UNION SOLUTION', '삼성 (SAMSUNG)', '숯불구이 천지연 · 송도갈비', '예지슬유치원', '쌍용종합사회복지관 · 쌍용어린이집', 'Coffee Mellow', '서울대정병원', '천안교차로', '충남개인택시조합', '하나마이크론 · 하나머티리얼즈', '천안 스노우의원', '나은필병원', '아이본안과', '순천향대학교', '천안시장애인종합복지관', 'CENO', '(주)케이엠티이엔지', '삼성미라클안과 천안'].map(n => `<div class="chip">${n}</div>`).join('')}</div><div class="grid2" style="margin-top:8px">${card('혜택 적용 방법', bul(['제휴업체 대상에 해당하시는 분은 미리 말씀해 주시면 확인 후 혜택을 적용해 드립니다.']))}${card('확인 절차', bul(['간단한 확인 절차 진행 후 협약에 따른 혜택이 적용됩니다.']))}</div></div>` },
  // ── 설명자료(explain): 임플란트 브랜드
  { slug: 'brand-osstem', eyebrow: '임플란트 브랜드 안내', title: '오스템 임플란트', sub: '국내 1위 브랜드 · SOI와 CA 표면 처리의 차이', body: `<div class="col">${card('SOI <span class="tag gold" style="margin-left:8px">Super Osseointegration</span>', pic('osstem_soi.jpg', 'TS III SOI 픽스처', 'contain h260') + bul(['빠르고 풍부한 혈병 형성으로 지혈 효과와 빠른 골 융합을 돕습니다.', 'SOI 코팅 표면 · 탁월한 골형성 단백질 부착', '초친수성 표면 · 치료 기간 단축']))}</div><div class="col">${card('CA <span class="tag" style="margin-left:8px">Calcium</span>', pic('osstem_ca.jpg', 'TS III CA 픽스처', 'contain h260') + bul(['칼슘 수용액의 표면 활성화 에너지로 치료 기간을 단축합니다.', '탁월한 혈액 젖음성 · 빠른 혈병 형성', '우수한 세포 유효성 · 우수한 골조직 형성']))}${card('비교', bul(['SOI는 CA의 장점에 골형성 단백질 부착력을 더한 최상위 라인입니다.', '두 제품 모두 오랜 기간 임상으로 검증된 오스템 정품입니다.']))}</div>` },
  ...
  ].map(([h, p]) => `<div class="tile"><h4>${h}</h4><p>${p}</p></div>`).join('')}</div></div><div class="col" style="flex:.9">${pic('straumann_blx.png', '스트라우만 BLX 임플란트 시스템 (Roxolid · SLActive)', 'contain h260')}${card('나사 풀림 · 파절', bul(['씹는 힘에도 결합력이 우수해 나사 풀림과 파절 현상을 최소화합니다.', '풀림이 생겨도 구조적으로 재조정·수리가 용이합니다.']))}${card('수명 · 뼈이식', bul(['뿌리(고정체)-기둥(지대주)-보철 구조로 정기검진 시 반영구적 사용이 가능합니다.', '뼈와 직접 결합되는 구조로 안정성이 우수하며, 필요 시 식립 전 또는 동시에 뼈이식이 가능합니다 (CT 확인 후 결정).', '뼈 손실이 적어 재식립이나 유지 관리에도 유리합니다.']))}</div>` },
]
