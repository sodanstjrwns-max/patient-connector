...
.row .ri.contain{object-fit:contain;padding:6px;background:radial-gradient(ellipse at 50% 40%,#fff 55%,#efe8df 100%)}
...
.pic.contain img{object-fit:contain;padding:14px;background:radial-gradient(ellipse at 50% 40%,#fff 55%,#efe8df 100%)}
...
const PAGES = [
  ...
  ])}<div class="note" style="margin-top:6px">서울비디치과는 기본적으로 지르코니아·세라믹 보철을 권해 드리며, 골드는 원하시는 분께 안내합니다.</div></div><div class="col" style="flex:.8">${card('골드 합금의 특징', bul(['잘 늘어나는 성질로 치아 삭제량이 적고 미세 부위를 잘 맞출 수 있으며, 교합력에 적응하는 좋은 재료입니다.', '일반 치과는 크라운 A type(금 46%)·A4G(33.6%), 인레이 P type(77.2%) 합금을 씁니다. 본원은 최고등급 크라운 S0 type(56%)·인레이 I type(83%)을 사용합니다.', '금속색이 보여 심미적이지 못한 단점이 있습니다.']))}${warr(WARR_GEN)}</div>` },
  // ── 앞니 보철
  { slug: 'anterior', title: '앞니 보철 비용', sub: '전치부 크라운 (치아 1개 기준)', body: `<div class="col">${rows([
    ...
    { img: 'zir_crowns_ant.jpg', fit: 'contain', name: '지르코니아 크라운', desc: '단단하고 심미적인 풀 지르코니아 크라운', price: 60 },
  ...
  { slug: 'ortho-1', title: '치아교정 비용', sub: '고정식 교정 · 부분 교정 · 악궁 확장 · 악정형', body: `<div class="col">${sec('고정식 교정 장치 치료')}${rows([
    { img: 'clippy_c.jpg', fit: 'contain', name: '클리피씨 (Clippy-C)', desc: 'Tomy(일본) 세라믹 자가결찰 브라켓', price: 500 }, { img: 'clarity_ultra.png', fit: 'contain', name: '클라리티 울트라 (Clarity Ultra)', desc: '3M(미국) 투명 세라믹 자가결찰 브라켓', price: 550 },
  ...
  { slug: 'tmj', title: '구강내과 비용', sub: '턱 보톡스 · 이갈이 장치', body: `<div class="col">${sec('턱 보톡스')}${rows([
    { img: 'wondertox.jpg', fit: 'contain', tag: '국내', name: '원더톡스', desc: '국내 생산 보툴리눔 톡신', price: 15 }, { img: 'xeomin.jpg', fit: 'contain', tag: '독일', tagGold: true, name: '제오민 (XEOMIN)', desc: '불순 단백질을 제거한 독일 프리미엄 보툴리눔 톡신', price: 35, hi: true },
  ])}</div><div class="col">${sec('이갈이 · 턱관절')}${rows([{ img: 'splint.jpg', fit: 'contain', name: '이갈이 장치 (스플린트)', desc: '맞춤 제작 · 월 진료비 10,000원 별도', price: 100 }])}${card('안내', bul(['이갈이 장치는 제작 후 정기 조정이 필요하며 월 진료비는 10,000원입니다.', '턱 보톡스는 개인별 근육량에 따라 용량이 조정될 수 있습니다.']))}</div>` },
  { slug: 'hygiene', title: '스테인 제거 · 지혈제', sub: '착색 제거와 발치·수술 시 사용하는 지혈 재료', body: `<div class="col">${sec('스테인 착색 제거')}${rows([{ img: 'ems_airflow.jpg', fit: 'contain', name: '에어플로우 (EMS AIRFLOW)', desc: '고운 파우더 분사로 커피·담배 착색 제거 · 1악당', price: 5 }])}<div class="note" style="margin-top:4px">EMS AIRFLOW Prophylaxis Master 장비를 사용합니다.</div></div><div class="col">${sec('지혈제')}${rows([
    { img: 'cutanplast.jpg', fit: 'contain', name: '큐탄플라스트 (젤라틴 지혈 스펀지)', desc: '무균 젤라틴 스펀지 · 혈전 형성을 도와 자연 응고 유도 · 표준 재료', price: 1 },
    { tag: '프리미엄', tagGold: true, name: '본트리 큐브 (콜라겐 지혈 스펀지)', desc: 'Type 1 콜라겐 + 식물성 키토산 · 우수한 지혈 · 99.9% 항균 · 낮은 알러지 위험', price: 7, hi: true },
  ])}</div>` },
  { slug: 'event', eyebrow: '이벤트 안내', title: '이달의 이벤트', sub: '기간과 조건은 데스크에서 확인해 주세요', body: `<div class="col">${sec('턱 보톡스')}${rows([{ img: 'wondertox.jpg', fit: 'contain', tag: 'EVENT', tagGold: true, name: '원더톡스 (국내)', was: 15, price: 4.9, hi: true }])}</div><div class="col">${sec('원데이 치아미백')}${rows([{ img: 'whitening.jpg', tag: 'EVENT', tagGold: true, name: '1회 · 30분', was: 30, price: 4.9, hi: true }, { tag: 'EVENT', tagGold: true, name: '2회 · 60분', was: 60, price: 8, hi: true }])}</div>`, footNote: '이벤트 가격은 해당 기간에만 적용됩니다.' },
  // ── 주의사항(notice)
