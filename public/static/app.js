  ...
  function renderHistory(main) {
    ...
    main.innerHTML = `
      ...
      <div class="bg-white border rounded-xl overflow-hidden">
        ...
        <tbody>${state.dispatches.length ? state.dispatches.map((d) => `<tr class="border-t"><td class="px-4 py-2 whitespace-nowrap text-slate-600">${esc((d.sent_at || d.created_at).slice(0, 16))}</td><td class="px-2 py-2 whitespace-nowrap">${esc(d.phone)}</td><td class="px-2 py-2 max-w-xs truncate" title="${esc(d.titles.join(', '))}">${esc(d.titles.join(', '))}</td><td class="px-2 py-2 text-slate-500">${esc(d.label || '')}</td><td class="px-2 py-2"><span class="px-1.5 py-0.5 rounded text-xs ${badge[d.status] || 'bg-slate-100'}" title="${esc(d.error || '')}">${label[d.status] || d.status}</span></td><td class="px-2 py-2 whitespace-nowrap">${d.first_opened_at ? `<span class="text-emerald-700">열람 ${d.open_count}회</span>` : '<span class="text-slate-400">아직</span>'}</td><td class="px-2 py-2 whitespace-nowrap"><button data-copy="${esc(d.url)}" class="text-slate-400 hover:text-slate-800" title="링크 복사"><i class="fa-solid fa-link"></i></button>${d.status === 'failed' && d.channel === 'alimtalk' ? ` <button data-resend="${d.id}" class="ml-2 text-xs px-2 py-0.5 rounded border text-slate-600 hover:bg-slate-50">재발송</button>` : ''}</td></tr>`).join('') : `<tr><td colspan="7" class="px-4 py-8 text-center text-slate-400">아직 보낸 안내장이 없습니다</td></tr>`}</tbody></table>
      </div>
    ...
    main.querySelectorAll('[data-resend]').forEach((b) => b.onclick = async () => { b.disabled = true; try { const r = await api('/dispatches/' + b.dataset.resend + '/resend', { method: 'POST' }); toast(r.ok ? '다시 보냈습니다' : ('실패: ' + (r.error || '')), r.ok); } catch (e) { toast(e.message, false); } await loadHistory(); render(); });
  }
