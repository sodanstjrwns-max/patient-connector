...
window.PC = {
  ...
  toast(msg, type = "ok") {
    ...
    el.className = "toast" + (type === "err" ? " error" : "");
    el.innerHTML = `<i class="fas ${type === "err" ? "fa-circle-exclamation" : "fa-check"}" aria-hidden="true"></i> &nbsp;${this.esc(msg)}`;
    box.append(el);
    ...
    return (
      {
        ...
        progression: ["질환 진행", "fa-layer-group"],
        cost: ["수가표", "fa-receipt"],
  ...
  typeBadge(type) {
    const [label, icon] = this.typeInfo(type);
    return `<span class="badge badge-neutral"><i class="fas ${icon}" aria-hidden="true"></i>${label}</span>`;
  },
  thumbOf(a) {
    return (
      a.media_urls?.[0] ||
      a.payload?.steps?.[0]?.image ||
      a.payload?.stages?.[0]?.image ||
      `/ph?t=${encodeURIComponent(a.title || "자료")}&v=${Number(a.id) || 0}`
    );
  },
  date(s, full = false) {
    if (!s) return "—";
  ...
  expired(s) {
    return (
      !s ||
      new Date(s.includes("T") ? s : s.replace(" ", "T") + "Z") < new Date()
    );
  },
  empty(title, sub, icon = "fa-folder-open") {
    return `<section class="empty-state"><i class="fas ${icon}" aria-hidden="true"></i><h3>${this.esc(title)}</h3><p>${this.esc(sub)}</p></section>`;
  },
  heading(kicker, title, sub, action = "") {
    return `<header class="page-heading"><div><p class="eyebrow" style="margin-bottom:7px;color:var(--accent)">${kicker}</p><h1>${title}</h1><p>${sub}</p></div>${action}</header>`;
  },
  footer() {
    return `<footer class="page-footer"><span>© Patient Connect · 더 나은 설명, 더 깊은 신뢰.</span><span>Made for the moments that matter.</span></footer>`;
  },
  shell(active, main) {
    const u = this.user,
  ...
  async copy(value) {
    try {
      ...
      this.modal(
        "링크 복사",
        `<p class="help-note">아래 링크를 선택해 복사해 주세요.</p><input class="input" readonly value="${this.esc(value)}" onclick="this.select()">`,
      );
    }
  },
  async share(url) {
    if (navigator.share) {
      try {
        await navigator.share({ title: "병원에서 보내드린 상담 안내", url });
      } catch (e) {
  ...
  modal(title, body, footer = "", small = false) {
    ...
    el.id = "pc-modal";
    el.innerHTML = `<section class="modal ${small ? "small-modal" : ""}" role="dialog" aria-modal="true" aria-labelledby="modal-title"><header class="modal-head"><h2 id="modal-title">${this.esc(title)}</h2><button class="icon-btn" aria-label="닫기" onclick="PC.closeModal()"><i class="fas fa-xmark"></i></button></header><div class="modal-body">${body}</div>${footer ? `<footer class="modal-footer">${footer}</footer>` : ""}</section>`;
  ...
  confirm(title, message, action, label = "확인") {
    this.modal(
      title,
      `<p class="help-note">${this.esc(message)}</p>`,
      `<button class="btn-ghost" onclick="PC.closeModal()">취소</button><button id="confirm-action" class="btn-primary">${this.esc(label)}</button>`,
      true,
    );
  ...
  async favorite(id, current, callback) {
    if (!this.user) {
      this.toast("로그인하면 즐겨찾기를 저장할 수 있습니다.");
      return;
    }
    await this.run(async () => {
      await axios.post(`/api/assets/${id}/favorite`, { favorite: !current });
      callback?.(!current);
    });
  },
  cover(a) {
    const url = this.thumbOf(a),
      real = url && !url.startsWith("/ph"),
      tone = (Number(a.id) || 0) % 4;
    if (real && a.type === "video")
    ...
    let shape = "";
    if (a.type === "cost")
      shape = `<rect x="40" y="17" width="122" height="137" rx="8" fill="#fff" opacity=".85"/><rect x="56" y="35" width="36" height="6" rx="3" fill="${ink}"/>${[63, 86, 109].map((y) => `<path d="M55 ${y}h88" stroke="${ink}" opacity=".23"/><rect x="56" y="${y + 8}" width="44" height="4" rx="2" fill="${ink}" opacity=".4"/><rect x="126" y="${y + 8}" width="17" height="4" rx="2" fill="${ink}"/>`).join("")}`;
    else if (a.type === "faq")
      shape = `<rect x="33" y="32" width="133" height="93" rx="20" fill="white" opacity=".75"/><path d="M65 125v19l24-19" fill="white" opacity=".75"/><text x="100" y="97" text-anchor="middle" font-family="Georgia" font-size="63" fill="${ink}">?</text><circle cx="167" cy="32" r="15" fill="${ink}" opacity=".15"/>`;
    else if (a.type === "steps" || a.type === "progression")
      shape = `<path d="M39 112Q70 20 112 70T175 40" fill="none" stroke="${ink}" stroke-dasharray="4 5" opacity=".65"/>${[
        [39, 112],
        [89, 63],
        [151, 76],
      ]
        .map(
          ([x, y], i) =>
            `<circle cx="${x}" cy="${y}" r="23" fill="white" opacity=".85"/><circle cx="${x}" cy="${y}" r="16" fill="none" stroke="${ink}" opacity=".4"/><text x="${x}" y="${y + 5}" text-anchor="middle" font-family="Georgia" font-size="17" fill="${ink}">${i + 1}</text>`,
        )
        .join("")}`;
    else
      shape = `<circle cx="100" cy="85" r="66" fill="white" opacity=".55"/><circle cx="100" cy="85" r="79" fill="none" stroke="${ink}" opacity=".2"/><path d="M70 65C61 33 83 30 100 42C117 30 139 33 130 65C123 90 117 97 116 122C112 142 103 132 105 111C105 93 95 93 95 111C97 132 88 142 84 122C83 97 77 90 70 65Z" fill="none" stroke="${ink}" stroke-width="3"/><path d="M85 51q15 8 30 0M155 46v16m-8-8h16" stroke="${ink}" stroke-width="2" opacity=".6"/>`;
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") PC.toggleSidebar(false);
});
// Icon-font glyphs are decorative; keep accessible button names human-readable.
const hideDecorativeIcons = () =>
  document
    .querySelectorAll("i:not([aria-hidden])")
    .forEach((el) => el.setAttribute("aria-hidden", "true"));
new MutationObserver(hideDecorativeIcons).observe(document.body, {
  childList: true,
  subtree: true,
});
hideDecorativeIcons();
